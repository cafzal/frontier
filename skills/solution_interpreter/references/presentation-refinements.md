...
- **A degenerate optimum makes a dual soft.** A solver-exact dual is exact for the basis the solver returned, but at a degenerate optimum (several constraints binding at one corner — a *basic* variable sitting on a bound) that basis is one of several, so the dual is one-sided and non-unique. It reads most precise exactly where it's least reliable. When the optimum looks degenerate, lead with the *ranking* of levers and present a single dual as a direction, not a quotable rate. This bites on the **LP** path; a strictly convex **QP** (mean-variance) has unique duals, so its read stays robust.
