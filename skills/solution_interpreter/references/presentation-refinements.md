...
- **The fraction is not a severity dial — read it with its `examples`, and with the shape.** Across the capture-batch corpus (larger, denser problems than the bundled examples) it runs 5–58% on QP overlays, 59–91% on linear allocation LPs (where most of it is whole-percent rounding: the `examples` show hundredths-of-a-unit shortfalls with no economic content), and 80–100% on `exact=true` MILP overlays where the same fraction carries real money (one corner understated a headline objective by 23% at the identical budget). The bundled examples sit lower on the same ordering (LP 0–43.6%, MILP 11–81%), so take the ordering — MILP above LP above QP, for reasons of shape — as the transferable part and the bands as that corpus's, not as priors for whatever you are looking at. A 90% on a MILP is a louder finding than a 90% on an LP, and a 40% on a QP can be noise. Open an `examples` entry, price the gap in the user's units, and let *that* set the tone — never map the percentage straight onto alarm.
- **On exact-K / tight-cardinality binary shapes, treat the exploratory frontier as a map rather than a candidate set.** When the model pins the number of picks (or nearly does), the feasible set is a lattice of discrete slates and the EA lands *near* the optimal ones rather than on them: 100% dominated shows up routinely (observed twice), and per-pick gaps of 13–25% are the normal reading, not a broken run. Say what it means — *"the exploration mapped the shape of the tradeoff; the exact pass is what produces the plans"* — and move the conversation to the certified points rather than defending the explored ones.

*Per-pick verdicts* — `per_pick` answers the question CERTIFY exists for: are the finalists **the user** curated optimal? One entry per pick, keyed by `content_signature` — a `verdict` (`optimal` | `dominated` | `infeasible` | `inconclusive`), a `gap`, and on a dominated pick `dominated_by`, the signature of the nearest certified point that beats it on every objective. The `gap` is the distance to that point: the worst per-objective shortfall as a fraction of the frontier's spread, so it reads on the same 0–1 scale as regret — an `optimal` pick has nothing above it, so its gap is 0. Each pick is judged at its slate's value under the **current** scores (the regime the certified frontier was built in), so a pin from before a score edit is ranked honestly rather than at its stale stored numbers — when that happens the entry carries `stored_values_stale` + `values_now`, and the move is to tell the user their pinned plan is now worth `values_now`, not what the pin remembers. `infeasible` means the pinned plan violates the constraints as they now stand — an invalid plan holds no place on any frontier, so unpin it and re-curate from the current frontier rather than ranking it. Absent when nothing is curated (as `regret.curated` is). Lead with it, by name, before the frontier-level numbers:
- *"Both finalists hold up — each is optimal at its own tradeoff, not just the best the search found."* / *"Your [custom_name] pick doesn't hold up: a certified plan beats it on every objective, by roughly [gap] of the frontier's range."*
- **A dominated finalist is a re-curation, not a footnote.** Hand `dominated_by` straight to `explore curate content_signature=<sig> source="exact"` — signatures resolve to a solution in the run you name, so there is no listing to scan — then unpin the beaten one with `remove=true`, and tell the user the shortlist changed and why. (`explore solutions content_signature=<sig> source="exact"` reads the replacement first if you want to describe it.) A shortlist you know is beaten is the one thing a certificate makes indefensible.
- `inconclusive` means the pin predates a current objective (`missing_objectives` names which), so it can't be placed on this frontier at all — re-curate it from the current run, and never read it as a pass.
- **`optimal` says the pick is unbeaten, not that it is a point on the exact frontier.** On continuous shapes the exact re-solve re-allocates, so an `optimal` pick's `content_signature` often resolves in the NSGA run and nowhere else — and every downstream ask keyed to it (`explore sensitivity` above all) fails on a signature lookup that reads like a bug. Take the anchor for duals from `explore solutions source="exact"` (or `tradeoffs source="exact"`) and pass *that* id or signature; keep the pick's own signature for curation, where it belongs.
...
*Frontier resolution* — `frontier_resolution` (2-objective continuous overlays only; `null` elsewhere — 3+ objectives, integer overlays, or objectives not in tension): the one block that reaches past the sampled points to the **true** frontier. The certified points' chords and their own shadow-price tangents sandwich the convex tradeoff curve, so `max_gap` is a guarantee about *every feasible plan at every level in the span*, not a spot-check — present the pre-built `claim` in the user's objective names: *"between [swept] [lo] and [hi], no feasible plan improves [primary] over straight-line interpolation of these points by more than [gap] ([fraction] of the frontier's span)."* This is the answer to "what about the points you didn't solve?" — the between-the-samples counterpart to the dominance check's at-the-samples answer, and the frontier-side sibling of `explore audit`'s whole-region guarantee. A small `fraction_of_frontier_range` is the headline ("the reported frontier is effectively the frontier"); a large one is honest coarseness — `widest_segment` names where, and a targeted exact solve there tightens it. Keep it scoped: a resolution bound, not global optimality — it bounds how much better *any* tradeoff between the anchors could be, it doesn't crown a point.
...
- **A false invariant (`invariant.holds = false`) is whole-percent rounding on a continuous shape — LP as well as QP.** `exact_dominated_by_nsga > 0` on a proportional problem means rounding the continuous optimum to whole percents edged it out (the `note` says so) — present it as a rounding footnote, not the EA beating the exact solve. Read the `note`, not the aggregation: the cause is the continuous-to-integer step, so a linear allocation LP shows it for exactly the reason a mean-variance QP does. (MILP corners, integer by construction, never show this.)
...
**A rate is a quotient, and `rate_guard` names the three ways one misleads.** Check every flag before quoting any number as a price:
...
- **`flat`** — the mirror case: the two solutions tie on the *numerator* objective (below `rate_guard.numerator_floor`), so the step bought essentially none of it and its rate is ~0. It misleads in the same direction for the opposite reason — a jump measured against ~0 explodes, so an unguarded detector plants its marker at the end of a **flat stretch** and the prose calls it a cliff. Keep the two reads apart for the user: `degenerate` is *"the price exploded"*, `flat` is *"nothing was bought"*. These are pre-filtered out of `inflection` (as the jump's baseline and as its landing) and out of `steepest_transitions` — on the `solver_exact_duals` basis a flat row's rate is the endpoint's shadow price and can be large, so without the filter a step that bought nothing tops the "most expensive steps" list. Among the detail rows, narrate one as a stretch where more [A] buys the user no [B] — often a real finding, and never a knee.
- Both floors are **relative to their pair**, read off that pair's own steps, so the same absolute step is a tie in one pair and a real move in another (a 0.15 step is nothing where the typical step is 4; a 0.08 step is real where the typical step is 0.5). The numerator floor sits inside its pair's bottom tail by construction, so a `flat` row is an outlier against its own neighbours — never merely the cheap side of a genuine elbow, however much cheaper that side is. Quote the floor beside the flag if a user challenges a filtered row, rather than arguing from how small the number looks.
...
**Lead with the median.** `summary` spans every transition including the guarded ones, so `rate_median` is the honest headline for "what does a unit of [A] typically cost?", while `rate_max` can still be a guarded row's artifact. Quote `steepest_transitions` for the genuinely expensive steps — that list is the filtered one, on both flags.

Inflection points also appear in `tradeoffs` output as `inflection_point_candidates` — these are the same solutions, pre-computed for convenience, and pre-filtered by the same tie guards on both sides of the quotient. Use them in the initial presentation alongside balanced and extremes, without requiring a separate `marginal_analysis` call.
...
**Check the staleness markers before reading anything back.** `scenario_results_stale` at the top of the payload means these runs were solved against an earlier model — present them as the last solved state and re-run `solve run_scenarios` before drawing conclusions, since a scenario "wipeout" or a shifted range is as likely an artifact of the edit as a finding. A per-scenario `constraint_overrides_stale` says that scenario's overrides are frozen at older base rules (they replace the base set wholesale, so the edit never reached it) — name it and offer to restate them. And when `regret` comes back `available: false` with `reason: "base_scenario_mismatch"`, the base and scenario runs describe different models: say the ranking is withheld rather than reporting a minimax pick from the rest of the payload.
...
- **With one scenario the risk block is an echo, not a tail** — `worst_case`, `best_case` and the CVaR all collapse onto that single scenario's value (the payload says so in `scenario_risk_note`). Three numbers agreeing is the arithmetic, not a robustness finding: never present it as "stable across futures." Name the degeneracy and route to the fix — *"there's only one future modelled here, so there's no spread to read; add a downside and an upside scenario and the tail numbers start meaning something."* (`expected` sits outside that collapse: it is probability-weighted, so a lone scenario carrying a probability scales it by that probability and it lands *below* the other three — a difference to explain, not a spread.)
