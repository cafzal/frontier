...
- Both floors are **relative to their pair** — that objective's own range and this frontier's own spacing — so the same absolute step is a tie in one pair and a real move in another (a 0.15 step is nothing where the typical step is 4; a 0.08 step is real where the typical step is 0.5). Quote the floor beside the flag if a user challenges a filtered row, rather than arguing from how small the number looks.
