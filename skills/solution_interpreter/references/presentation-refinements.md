...
*Per-pick verdicts* — `per_pick` answers the question CERTIFY exists for: are the finalists **the user** curated optimal? One entry per pick, keyed by `content_signature` — a `verdict` (`optimal` | `dominated` | `inconclusive`), a `gap`, and on a dominated pick `dominated_by`, the signature of the nearest certified point that beats it on every objective. The `gap` is the distance to that point: the worst per-objective shortfall as a fraction of the frontier's spread, so it reads on the same 0–1 scale as regret — an `optimal` pick has nothing above it, so its gap is 0. Absent when nothing is curated (as `regret.curated` is). Lead with it, by name, before the frontier-level numbers:
