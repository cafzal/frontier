...
- **A dominated finalist is a re-curation, not a footnote.** Match `dominated_by` in `explore solutions source="exact"` (every solution echoes its `content_signature`), curate that point in its place, and tell the user the shortlist changed and why. A shortlist you know is beaten is the one thing a certificate makes indefensible.
