...
- **The fraction is not a severity dial — read it with its `examples`, and with the shape.** Measured across the bundled examples it runs 5–58% on QP overlays, 59–91% on linear allocation LPs (where most of it is whole-percent rounding: the `examples` show hundredths-of-a-unit shortfalls with no economic content), and 80–100% on `exact=true` MILP overlays where the same fraction carries real money (one corner understated a headline objective by 23% at the identical budget). So a 90% on a MILP is a louder finding than a 90% on an LP, and a 40% on a QP can be noise. Open an `examples` entry, price the gap in the user's units, and let *that* set the tone — never map the percentage straight onto alarm.
