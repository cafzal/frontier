...
**Solver-exact explainability:** [budget allocation](budget_allocation/#explainability--shadow-prices--near-misses) (purely linear **LP**) and the [investment portfolio](investment_portfolio/#explainability--shadow-prices--near-misses) (mean-variance **QP**) both walk through `solve solver="highs"` → `explore sensitivity` — shadow prices ("where to invest") and reduced costs ("near-misses") read straight from the optimizer on the continuous (LP / QP) paths. (Binary MILP shapes have no duals and fall back to the frontier-inferred estimate.)
