...
The default solve path is pymoo's NSGA-II/III for all problem shapes. Two **opt-in exact-solver backends** sit alongside it, gated so the default path never imports either (each imports its solver lazily, so the modules load whether or not that solver is present). Both share **one NSGA-scalarization engine** ([`solvers/_scalarization.py`](solvers/_scalarization.py)): NSGA-II — the evolutionary **explorer** — evolves epsilon-constraint scalarization targets (and, under a cardinality/group cap, an asset-selection priority vector), while an injected **inner solver** solves each scalarized single-objective problem to optimality; the Pareto frontier is assembled from those exact inner optima. This is **approximate exploration, exact follow-on**: the same evolutionary search that runs alone by default decides *where* to look on the frontier, and the exact inner solve **grounds** each explored point to optimal instead of heuristic — epsilon-constraint scalarization, driven adaptively by the EA rather than swept over a fixed grid. The two backends differ **only** in that injected inner solve, and both return the engine's exact `Run` / `Solution` shape — so explorer, metrics, and store need zero changes. Additive, gated, reversible.
