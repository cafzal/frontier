...
| Skill | Purpose |
|-------|---------|
| **problem_framing** | Translate decision language into objectives/options/constraints. Covers objective vs constraint classification (principle-based, not keyword matching), hidden objective detection, approach selection (binary vs proportional – "does quantity matter?"), aggregation modes (canonical definition – sum/avg/min/max/quadratic), interaction matrices for quadratic aggregation, reference points, scenario definition, question anchors for guiding problem exploration. Cross-referenced by other skills. The whole-model pre-solve cross-check (unstated goals, orphaned elements, redundant constraints) lives in `references/formulation-validation.md`, fetched as the *Pre-Solve Validation* section. |
| **data_collection** | Guide score elicitation. Covers data readiness levels, anchoring techniques, batch efficiency, source evaluation, conflict resolution, score quality signals (variance, scale mismatch), aggregation implications on scoring (cross-references problem_framing), score provenance/attribution, completeness drive. The extraction-tier → confidence detail for researching scores from messy sources lives in `references/sourcing.md`, fetched as the *Extraction Tiers* section. |
| **optimization_strategy** | Drive solve progression. Core: iteration expectations, validate→run→examine flow, background-job literacy, mode + solver selection (incl. the staged exact-certify trigger and engine table), constraint strategy (cross-references problem_framing for types), infeasibility response, binding constraint detection, curated solution survival tracking, run comparison, stale results and re-run judgment. Deep exact-solver narration (claim calibration, gate declines + redefines, `exact=true`) lives in `references/exact-solvers.md`, fetched as the *Exact Solvers — Depth* section; constraint *interaction* patterns (cascading tightness, contradictory, redundant, symmetry) live in `references/constraint-interaction.md`, fetched as the *Constraint Interaction Patterns* section. |
